...
class AdminToolsController extends Controller
{
    ...
    public function complain_detail(Request $req)
    {
        ...
        // ->join('users', 'users.id', '=', 'complains.complainer_id')
        // ->where('complains.complainer_id', '=', $req->id) // Find posts by user with ID 1
        // ->select('posts.*', 'users.name as user_name') // Select columns from both tables
        // ->get();
    


        $complain = Complain::with('user')->find($req->id);
        // return $complain;
